...
void affichage(sf::RenderWindow *fenetre, struct noeud *graphe, int nbre_points, struct link *liens, int nbre_liens) {
	// préparation des couleurs
	unsigned int index_points = 0;
	unsigned int index_liens = 0;
	unsigned int temp_depart = 0, temp_fin = 0;
	const sf::Color noir = { 0,0,0,0 }, blanc = { 255,255,255,255 }, gris = { 128,128,128,128 };
	sf::CircleShape **cercle = (sf::CircleShape**) malloc(nbre_points*sizeof(sf::CircleShape*));
	sf::Shape **ligne = (sf::Shape**) malloc(nbre_liens*sizeof(sf::Shape*));

	fenetre->clear(noir);
	// draw circles
	for (index_points = 0; index_points < nbre_points; index_points++)
	{
		cercle[index_points]->setPosition(graphe[index_points].posx, graphe[index_points].posy);
		cercle[index_points]->setRadius(10);
		cercle[index_points]->setFillColor(blanc);
		cercle[index_points]->setOutlineThickness(1);
		cercle[index_points]->setOutlineColor(gris); // = (graphe[index_points].posx, graphe[index_points].posy, 10, blanc, 1, gris);
		fenetre->draw(cercle[index_points]);  //sfRenderWindow_DrawShape(fenetre, cercle[index_points]);  // tracer le cercle1
